...
async function fetchFriendList(page, pageNum = 1) {
...
async function findFriendByUid(page, encryptUid) {
    const friends = await fetchFriendList(page);
    return friends.find((f) => f.encryptUid === encryptUid) || null;
}
async function clickFriendInList(page, friendName) {
...
async function typeAndSendMessage(page, text) {
    // 输入消息
